...
test('pay-in-full checkout allows promotion codes and does not include recurring data', () => {
...
test('book checkout adds taxable book line with public image URL', () => {
  ...
  assert.equal(
    params.get('line_items[1][price_data][product_data][images][0]'),
...
test('unknown class and payment selections are rejected', () => {
  ...
  assert.equal(
    _test.validateSelection({ classId: 'fall-2026-thu-beg', paymentType: 'weekly', includeBook: false }).error.body,
